...
const Select = ({ suggesstions, updateList }: Props) => {
     ...
     return (
          <div className='multi-select-container'>
               ...
               <div onClick={() => { setShowlist(true) }} ref={containerRef}               >
                    <div>
                         <TextField 
                              fullWidth 
                              style={{marginTop: "2px"}} 
                    ...
                    {<ul ref={listRef} className={`suggesstion-list ${showList ? "active" : ""}`}>
                         {(text.length ? filterSuggesstion : suggesstions).map((suggesstion, index) => (
                              <li key={index} style={{listStyle: 'none'}} data-list-item className={`${index === listIndex ? 'focus-li' : ''}`} onClick={(e) => listClick(e, suggesstion)}>{suggesstion}</li>
                         ))}
                    </ul>}
               </div>
